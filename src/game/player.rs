...
impl Player {
    ...
    pub fn hand(&self) -> [u8; MAX_RANK] {
        self.hand
    }

    pub fn rank_count(&self, rank: u8) -> u8 {
        check_rank(rank);
        self.hand[(rank - 1) as usize]
    }
    ...
    pub fn add_cards(&mut self, rank: u8, count: u8) {
        ...
        check_count(count);
        assert!(
            count + self.hand[(rank - 1) as usize] <= MAX_COUNT_PER_RANK as u8,
        ...
        self.hand[(rank - 1) as usize] += count;
    }
    ...
    pub fn remove_cards(&mut self, rank: u8, count: u8) {
        ...
        check_count(count);
        assert!(
            count <= self.hand[(rank - 1) as usize],
